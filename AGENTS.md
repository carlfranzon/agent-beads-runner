...
- **C&C command style**: Slash commands are first-class (`/go`, `/start`, `/agent`, `/model`, `/beads`, `/bead`, `/help`, `/exit`, `/quit`), with legacy `run/stop/kill/status/help/exit/quit` retained for compatibility. When adding C&C commands, update `run_cc` dispatch, add a `cc_cmd_*` helper, update `cc_cmd_help`, and document in README/SKILL.
