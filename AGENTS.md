...
- **`/review` scope selector**: `/review` must prompt for scope with `next PR` (oldest open PR only) or `all open PRs` (oldest to newest). `cc_cmd_slash_review()` forwards this choice via `--review-scope`.
