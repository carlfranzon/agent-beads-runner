...
| `--review --parallel-N` | Start N review workers that claim PRs and drain all open agent PRs safely |
...
| `/review` | Choose scope: `next PR` launches one reviewer; `all open PRs` prompts for agent count, then launches that many queue-consuming review workers until all open agent PRs are reviewed. |
